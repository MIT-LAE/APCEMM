...
namespace EPM
{
    int Integrate( RealDouble temperature_K, RealDouble pressure_Pa, RealDouble relHumidity_w, RealDouble varArray[], RealDouble fixArray[], const Aircraft &AC, const Emission &EI )
    {
        ...
        RealDouble delta_z = AC.getVortexdeltaz1(); 
    ...
    int RunMicrophysics( RealDouble temperature_K, RealDouble pressure_Pa, RealDouble relHumidity_w, RealDouble varArray[], RealDouble fixArray[], const Aircraft &AC, const Emission &EI, RealDouble delta_T_ad, RealDouble delta_T )
    {
        ...
        std::vector<RealDouble> SO4_rJ( SO4_NBIN, 0.0 );
        std::vector<RealDouble> SO4_rE( SO4_NBIN+1, 0.0 );
        std::vector<RealDouble> SO4_vJ( SO4_NBIN, 0.0 );
        ...
        const AIM::Coagulation Kernel( "liquid", SO4_rJ, physConst::RHO_SO4, 1.0E-05, 1.0E+03, temperature_K, pressure_Pa );
...
//        RealDouble sSO4 = rSO4/10; /* For normal distributions, sSO4 */
        ...
        RealDouble ambSoot = 0.05; /* [#/cm^3] */
        RealDouble varSoot = ambSoot + EI.getSoot() / ( 4.0 / RealDouble(3.0) * physConst::PI * physConst::RHO_SOOT * 1.00E+03 * EI.getSootRad() * EI.getSootRad() * EI.getSootRad() ) * AC.getFuelFlow() / RealDouble(AC.getEngNumber()) / AC.getVFlight() * 1.00E-06; /* [ #/cm^3 ] */
...
//        {
//            
//            inline void operator()( const Vector_1D &x, Vector_1D &dxdt, const RealDouble t = 0 ) const
//            {
//
//                /* DESCRIPTION:
...
//                 */
//
//                dxdt[0] = x[0]; //- dilutionRatio( t ) * ( x[0] );
//
//            } /* End of operator() */
//
//        };
...
//        RHS( x, dxdt, 0 );
        ...
        EPM::odeSolver<void ( const Vector_1D&, Vector_1D&, RealDouble )> ode( odeRHS, x, adaptiveStep ); 
        //EPM::odeSolver<void ( const Vector_1D&, Vector_1D&, RealDouble, const RealDouble )> ode( odeRHS, x, adaptiveStep );
//        EPM::odeSolver<void (*)( const Vector_1D&, Vector_1D&, RealDouble )> ode( gas_aerosol_rhs(), x, adaptiveStep, 0, 0.0 );
       // EPM::odeSolver<EPM::RunMicrophysics(RealDouble, RealDouble, RealDouble, RealDouble*, RealDouble*, const Aircraft&, const Emission&, RealDouble, RealDouble)::gas_aerosol_rhs> ode(gas_aerosol_rhs(), x, adaptiveStep, 0, 0.0 );
//        gas_aerosol_rhs RHS;
        //EPM::odeSolver<gas_aerosol_rhs> ode(gas_aerosol_rhs(), x, adaptiveStep, 0, 0.0 );
//        EPM::odeSolver<gas_aerosol_rhs> ode(gas_aerosol_rhs(), x, adaptiveStep, 0, 0.0 );
 
        //EPM::odeSolver ode( adaptiveStep, 0, 0.0 );
        Vector_2D obs_Var;
        Vector_1D obs_Time;
        EPM::streamingObserver observer( obs_Var, obs_Time, "/home/fritzt/CAPCEMM/data/Micro.out", 1 );

        UInt steps = ode.integrate( 0.0, 1.0, 0.01, observer );

//        UInt steps = ode.integrate( gas_aerosol_rhs(), x, 0.0, 1.0, 0.01, observer );
//        UInt steps = ode.integrate( RHS, x, 0.0, 1.0, 0.01, observer );
    ...
    RealDouble temp_Vortex( const RealDouble time, const RealDouble delta_T )
    {
         ...
         *
         * OUTPUT:
         * - RealDouble :: temperature increase from cruise altitude in K */
    ...
    RealDouble dilutionRatio( const RealDouble t )
    {
        ...
        if ( ( t >= t_0 ) && ( t <= t_j ) ) {
            ...
            dRat = ( 0.8 / t_1 ) / ( 1.0 + Cv * ( 0.8 / t_1 ) * ( t_2 - t_1 ) ) / ( 1.0 + 1.0 / ( 1.0 + m * exp( - ( t - t_2 ) / ( n * t_2 ) ) ) * ( t - t_2 ) );
        } else {
            dRat = 0;
        }
    ...
    void odeRHS( const Vector_1D &x, Vector_1D &dxdt, const RealDouble t )
    {
