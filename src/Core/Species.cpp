                                               ...
                                               const unsigned int iNt ) const
{
    ...
    for ( iRing = 0; iRing < nRing; iRing++ ) {
        area = ringArea[iRing];
        ringAverage[iSpec] += CO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += PPN[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += BrNO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += IEPOX[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += PMNN[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += N2O[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += N[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += PAN[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ALK4[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MAP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MPN[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += Cl2O2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ETP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += HNO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += C3H8[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += RA3P[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += RB3P[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += OClO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ClNO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ISOP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += HNO4[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MAOP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ClOO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += RP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += BrCl[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += PP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += PRPN[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += SO4[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += Br2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ETHLN[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MVKN[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += R4P[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += C2H6[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += RIP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += VRP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ATOOH[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += IAP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += DHMOB[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MOBA[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MRP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += N2O5[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ISNOHOO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ISNP[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ISOPNB[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += IEPOXOO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MACRNO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ROH[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MOBAOO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += DIBOO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += PMN[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ISNOOB[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += INPN[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += H[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += BrNO3[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += PRPE[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MVKOO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += Cl2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ISOPND[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += HOBr[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += A3O2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += PROPNN[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += GLYX[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MAOPO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += CH4[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += GAOO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += B3O2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ACET[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MACRN[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += CH2OO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MGLYOO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += VRO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MGLOO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MACROO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += PO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += CH3CHOO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MAN2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ISNOOA[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += H2O2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += PRN1[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ETO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += KO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += RCO3[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += HC5OO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += GLYC[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ClNO3[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += RIO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += R4N1[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += HOCl[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ATO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += HNO3[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ISN1[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MAO3[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MRO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += INO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += HAC[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += HC5[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MGLY[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ISOPNBO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ISOPNDO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += R4O2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += R4N2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += BrO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += RCHO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MEK[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ClO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MACR[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += SO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MVK[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += ALD2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MCO3[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += CH2O[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += H2O[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += Br[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += NO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += NO3[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += Cl[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += O[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += O1D[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += O3[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += HO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += NO2[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += OH[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += HBr[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += HCl[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += CO[iNt][iRing] * area;
        iSpec++;
        ringAverage[iSpec] += MO2[iNt][iRing] * area;
    }

    for ( iSpec = 0; iSpec < NVAR; iSpec++ )
        ringAverage[iSpec] /= totArea;
    return ringAverage;
