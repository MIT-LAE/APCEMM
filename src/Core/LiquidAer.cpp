                        ...
                        std::vector<double> &NDENS_AER , std::vector<double> &SAD_AER)
{
     ...
     * - index         : Time index of ambient data
    ...
    /* Mass of dry air per grid box [kg/m] */
    const double INVAIR = MW_Air / ( airDens * 1.0E+06 * Area ); 
    ...
    /* Available NO3 mass */
    KG_NO3 = Data.HNO3[index] * MW_NIT / MW_HNO3; // + Data.NIT[index];

    /* Mass of HNO3 per grid box [kg/m] */
    HNO3SUM  = Data.HNO3[index] * INVAIR / MW_HNO3;
    HNO3SUM += 0; // Data.NIT[index] * INVAIR / MW_NIT;

    /* Mass of H2O per grid box [kg/m] */
    H2OSUM = Data.H2O[index] * INVAIR / MW_H2O;
    ...
    if ( LSOLIDPSC ) {
        if ( STATE_LOCAL == 3 ) {
            ...
            KG_ICE    = H2O_BOX_S * MW_H2O / INVAIR;
            /* Unit check:
             *        = mole      * kg/mol / (kg/m)
             *        = kg / m */
        ...
        if ( ( HNO3_BOX_S > 0.0E+00 ) && ( STATE_LOCAL >= 2 ) ) {
             ...
             *      = mole       * molec/cm^3 * cm^3/m^3 * kg/mol / ( kg/m^3 ice         * molec/mole   )
             *      = m^3 ice / m^3 air*/
            KG_NAT  = HNO3_BOX_S * MW_NAT / INVAIR;
            /* Unit check:
             *        = mole      * kg/mol / (kg/m)
             *        = kg / m */
    ...
    if ( LSOLIDPSC && IS_STRAT ) {
        ...
        Data.NIT[index] = KG_NAT * MW_NIT / MW_NAT; /* [kg/m] */

        /* Remove ( kg NO2 as NAT ) from total kg NO3
         * then convert to kg HNO3 */
        Data.HNO3[index] = ( KG_NO3 - Data.NIT[index] ) * MW_HNO3 / MW_NIT;
    ...
    H2SO4SUM = Data.SO4T[index]  * INVAIR / MW_SO4;
    BrNO3SUM = Data.BrNO3[index] * INVAIR / MW_BrNO3;
    ClNO3SUM = Data.ClNO3[index] * INVAIR / MW_ClNO3;
    HOClSUM  = Data.HOCl[index]  * INVAIR / MW_HOCl;
    HClSUM   = Data.HCl[index]   * INVAIR / MW_HCl;
    HOBrSUM  = Data.HOBr[index]  * INVAIR / MW_HOBr;
    HBrSUM   = Data.HBr[index]   * INVAIR / MW_HBr;

    /* Consider gaseous H2SO4 to be unavailable for SLA */
    H2SO4_gFRAC = H2SO4_GASFRAC( temperature_K, H2SO4SUM );
    ...
    if ( !IS_STRAT ) {
        ...
        if ( STATE_LOCAL == 0 ) {
            ...
            W_H2O = W_H2O + W_HNO3;
            W_HNO3 = 0.0E+00;
            HNO3_BOX_G = HNO3_BOX_S - HNO3_BOX_S;
            HNO3_BOX_L = 0.0E+00;
    ...
    if ( ( HNO3SUM > 1.0E+00 ) && ( SafeDiv( HNO3_BOX_L, HNO3SUM ) ) )
        AERFRAC[1] = HNO3_BOX_L / HNO3SUM;

    if ( ( HClSUM > 1.0E+00 ) && ( SafeDiv( HCl_BOX_L, HClSUM ) ) )
        AERFRAC[2] = HCl_BOX_L / HClSUM;

    if ( ( HOClSUM > 1.0E+00 ) && ( SafeDiv( HOCl_BOX_L, HOClSUM ) ) )
        AERFRAC[3] = HOCl_BOX_L / HOClSUM;

    if ( ( HBrSUM > 1.0E+00 ) && ( SafeDiv( HBr_BOX_L, HBrSUM ) ) )
        AERFRAC[4] = HBr_BOX_L / HBrSUM;

    if ( ( HOBrSUM > 1.0E+00 ) && ( SafeDiv( HOBr_BOX_L, HOBrSUM ) ) )
        AERFRAC[5] = HOBr_BOX_L / HOBrSUM;

    if ( ( H2OSUM > 1.0E+00 ) && ( SafeDiv( H2O_BOX_L, H2OSUM ) ) )
