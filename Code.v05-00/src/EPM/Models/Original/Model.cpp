...
std::variant<EPM::Output, SimStatus> Original::run() {
    ...
    // This sets the values in VAR_ and FIX_ to the values in the solution data
    // structure at indices i, j
    epmSolution.getData(VAR_, FIX_, i_0, j_0);

    // Run EPM.
    return Integrate(epmSolution.getAerosol());
}
